...
The `v0.2.6` prerelease flow also exposes a BLE fallback direct-read interval for profiles that keep a GATT session open, from `5` to `300` seconds (`10` seconds by default).
...
- INT-12E-BW support is currently prerelease-only. It provides read-only BLE temperature/battery updates, applies valid notifications immediately and uses configurable direct GATT reads as a fallback. The station may end sessions after about 30 seconds, after which the integration reconnects automatically. Tuya LAN, cloud, writes and protocol-state entities remain disabled.
